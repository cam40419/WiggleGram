...
def list_synced_sets(base_dir: str):
    ...
    # Use cam0 as reference for filenames
    ...
    if len(sets) < 5:
        # Not a hard requirement, but typically you want ~10–20+ for good calibration.
...
def find_corners(image, pattern_size):
    ...
    # More robust than classic findChessboardCorners in many cases:
...
def build_object_points(pattern_size, square_size):
    ...
    cols, rows = (
        pattern_size  # OpenCV uses (columns, rows) i.e. inner corners per row/col
    )
...
def calibrate_four_cameras(
    ...
    # 1) Calibrate each camera intrinsics independently
    Ks = []
    dists = []
    rvecs_all = []
    tvecs_all = []
    ...
    # 2) Calibrate extrinsics between cam0 and each other cam using stereoCalibrate
